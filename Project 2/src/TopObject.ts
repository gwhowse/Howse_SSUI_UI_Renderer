...
export class TopObject extends DrawnObjectBase {
    ...
    public override damageArea(xv: number, yv: number, wv: number, hv: number): void {
        ...
        if (this._damaged) {
            ...
            let xRightOld : number = this._damageRectX + this._damageRectW;
            let yBottomOld : number = this._damageRectY + this._damageRectH;

            // Then get the bottom right corner of our newly declared damage
            let xRightNew : number = xv + wv;
            let yBottomNew : number = yv + hv;
