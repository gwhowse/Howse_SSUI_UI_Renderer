...
export class Column extends Group { 
    ...
    protected override _doLocalSizing() : void {
        //=== YOUR CODE HERE ===
        // set up our cumulative variables to track max width and total height
        let maxW : SizeConfigLiteral = SizeConfig.fixed(0);
        let sumH : SizeConfigLiteral = SizeConfig.fixed(0);

        // loop through all of the children, taking sum of height and max of width
    ...
    protected _measureChildren() : [number, number, number] {
        ...
        for (let child of this.children) {
            ...
            if (child instanceof Spring) {
                ...
                // This is a column so we should get each child's height configuration
                let config = child.hConfig;
    ...
    protected override _completeLocalLayout() : void { 
        ...
        if (this.wJustification === "left") {
            // everything starts at x=0
            for (let child of this.children) {
                child.x = 0;
            }
        } else if (this.wJustification === "right") {
            // everything ends at our right edge, which is just width
            // we need left edge though, so we subtract off each child's width to find it's left x
            for (let child of this.children) {
                child.x = this.w - child.w;
            }
        } else {
            // everything needs to be centered at xCenter
            // so each child's left edge is xCenter - half of it's width
            let xCenter = this.w / 2;
            for (let child of this.children) {
                child.x = xCenter - (child.w / 2);
            }
        }
    }
