...
export class TextObject extends DrawnObjectBase {
    ...
    _recalcSize(ctx) {
        //=== YOUR CODE HERE ===
        // get the measurements of our text given the current font, etc.
        let meas = this._measureText(this.text, this.font, ctx);
    ...
    _drawSelfOnly(ctx) {
        ...
        try {
            ...
            //=== YOUR CODE HERE ===
            // Make sure our font is correct
            ctx.font = this.font;
            // xv should be whatever our padding width is
            let xv = this.padding.w;
            // yv should be our height padding + baseline
            let meas = this._measureText(this.text, this.font, ctx);
            let yv = this.padding.h + meas.baseln;
            if (this.renderType === 'fill') {
                ctx.fillStyle = clr;
                ctx.fillText(this.text, xv, yv);
            }
            else {
                ctx.strokeStyle = clr;
                ctx.strokeText(this.text, xv, yv);
            }
        }
        finally {
